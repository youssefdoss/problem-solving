...
def rucksack_reorganization():
    ...
    wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn ttgJtRGJQctTZtZT CrZsJsPPZsGzwwsLwLmpwMDw The
    first rucksack contains the items vJrwpWtwJgWrhcsFMMfFFhFp, which means its
    first compartment contains the items vJrwpWtwJgWr, while the second
    ...
    compartments only share item type s. To help prioritize item rearrangement,
    every item type can be converted to a priority:

    Lowercase item types a through z have priorities 1 through 26. Uppercase
    item types A through Z have priorities 27 through 52. In the above example,
    the priority of the item type that appears in both compartments of each
    ...
    for rucksack in rucksacks:
        ...
        for char in compartment1:
            if char in compartment2:
                prio_sum += calculate_priority(char)
                break
...
def calculate_priority(char):
    '''
    Calculates the priority of a given character.
    '''
    if 'a' <= char <= 'z':
        return ord(char) - 96
    elif 'A' <= char <= 'Z':
...
def identify_badges():
    ...
    wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn ttgJtRGJQctTZtZT CrZsJsPPZsGzwwsLwLmpwMDw In
    the first group, the only item type that appears in all three rucksacks is
    ...
    for i in range(0, len(rucksacks), 3):
        curr_rucksacks = rucksacks[i:i+3]
        item_sets = [set(rucksack) for rucksack in curr_rucksacks]
        common_items = set.intersection(*item_sets)
        badge = common_items.pop()
        prio_sum += calculate_priority(badge)
