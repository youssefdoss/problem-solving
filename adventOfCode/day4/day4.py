...
def count_full_overlap():
    ...
    2-4,6-8 2-3,4-5 5-7,7-9 2-8,3-7 6-6,4-6 2-6,4-8 For the first few pairs,
    this list means:
    ...
    .23456...  2-6 ...45678.  4-8 Some of the pairs have noticed that one of
    their assignments fully contains the other. For example, 2-8 fully contains
    3-7, and 6-6 is fully contained by 4-6. In pairs where one assignment fully
    contains the other, one Elf in the pair would be exclusively cleaning
    sections their partner will already be cleaning, so these seem like the most
    ...
    for pair in pairs:
        ...
        if ((int(range1[0]) <= int(range2[0]) and int(range1[1]) >= int(range2[1])) or
            (int(range1[0]) >= int(range2[0]) and int(range1[1]) <= int(range2[1]))):

            count += 1

    print(count)

def count_partial_overlap():
    '''
    ...
    for pair in pairs:
        ...
        if (int(range2[0]) <= int(range1[0]) <= int(range2[1]) or
            int(range2[0]) <= int(range1[1]) <= int(range2[1]) or
            int(range1[0]) <= int(range2[0]) <= int(range1[1]) or
            int(range1[0]) <= int(range2[1]) <= int(range1[1])):
